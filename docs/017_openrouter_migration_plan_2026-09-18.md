...
### 10.7 배포 선행 조건 — `OPENROUTER_API_KEY_ADMISSIONS` (미해결)
...
**2026-09-19 확인: dev blob 에는 `_WRITING`·`_CLASSBOT` 만 있고 `_ADMISSIONS` 는 없다.**
오너 결정 — **dev 등록은 보류, 운영 전환 시 등록한다.**

⚠️ 그래서 **지금 상태로 머지·배포하면 dev 진단이 전부 실패한다.** 머지 전에 둘 중 하나를 골라야 한다.
1. dev 에도 키를 등록한다(권장 — gemini 경로를 dev 에서 한 번도 검증하지 못한 채 운영에 올리는 것을 피한다).
2. dev 환경에 `ADMISSIONS_LLM_GATEWAY=anthropic` 을 설정해 haiku-4.5 직결로 둔다(키 불필요).
   대신 dev 에서 검증되는 경로와 운영 경로가 달라진다.
