  ...
  const res = await anthropic.messages.parse({
    ...
    messages: [{
      ...
      content:
        ...
        `진단 보완지점: ${diagnosis.criteria.map(c => `${c.key}:${c.gaps.map(g => g.title).join('·')}`).join(' / ')}\n\n` +
        `생기부(마스킹됨):\n${profile.saengbu}\n\n` +
        `보완지점을 메울 "학생이 앞으로 할" 활동을 제안하라. recordArea는 SETUK·CREATIVE_REGULAR·BEHAVIOR만 사용하고, 각 후보에 생기부 인용(evidence)을 포함하라.\n` +
        `- estimatedMinutes: 학생이 혼자 수행하는 데 드는 예상 시간(분, 10~240).\n` +
        `- linkedQuestions: 이 활동이 대비하는 면접 질문 번호(["Q1"] 형식). 없으면 빈 배열.\n` +
