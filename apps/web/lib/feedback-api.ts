...
import { requestPinned, type PinnedRequestFn } from '@/lib/pinned-request';
...
export interface FeedbackApiPayload {
   ...
   */
  context: {
    /** 제출 시점 경로(`pathname`). 호스트도 쿼리도 담지 않는다(@pullim/shared 가 정제한다). */
    pageUrl?: string;
...
export interface FeedbackApiTarget {
  ...
  /**
   * **연결할 IP.** 위 주소를 확인할 때 해석된 그 주소다 — 두 요청 모두 이 IP 에 연결을 고정한다.
   * 확인만 하고 이름으로 다시 연결하면(=fetch) 그 사이에 응답이 바뀌는 DNS 리바인딩을 막지
   * 못하고, 자격증명을 싣는 요청에서는 그게 곧 secret 유출이다. TLS(SNI·인증서 검증)는
   * 호스트명 기준 그대로다.
   */
  address: string;
 ...
 *
 * ③의 해석 결과는 **버리지 않고 `target.address` 로 들고 나간다.** 확인만 하고 이름으로 다시
 * 연결하면(=fetch) 그 사이에 응답이 바뀌는 리바인딩이 열린 채다 — 저장·신원 확인 두 요청 모두
 * 이 IP 에 연결을 고정한다(lib/pinned-request.ts).
 */
export async function resolveFeedbackApiTarget(): Promise<FeedbackApiTargetResult> {
  ...
  if (isLoopbackHost(root.hostname)) {
    // 리바인딩으로 갈 수 있는 "더 나쁜 곳"이 없다(이미 이 기계다). DNS 를 타지 않고 고정한다 —
    // 로컬의 `localhost` 해석 결과는 환경마다 갈린다.
    address = loopbackAddress(root.hostname);
  } else {
    ...
    // 그리고 그 주소를 **버리지 않고 연결 대상으로 들고 간다** — 확인과 연결이 같은 IP 를 봐야
    // 리바인딩이 닫힌다.
    const resolvedAddress = await resolvePublicAddress(root.hostname);
    if (!resolvedAddress) return { ok: false, reason: 'not-allowed' };
  ...
  return { ok: true, target: { endpoint, identityEndpoint, address, serviceKey } };
}

/**
 * loopback 이름 → 연결할 주소. 이름이 이미 IP 면 그대로, `localhost` 류는 IPv4 루프백으로 둔다.
 * DNS 를 타지 않는 이유: 로컬 해석 결과는 환경마다 갈리고, 어느 쪽이든 같은 기계 안이라
 * 고정해서 얻을 보안 이득이 없다(형식만 맞추면 된다).
 */
function loopbackAddress(hostname: string): string {
  const host = hostname.toLowerCase().replace(/^\[/, '').replace(/\]$/, '');
  if (host === '::1') return '::1';
  return /^127\.\d{1,3}\.\d{1,3}\.\d{1,3}$/.test(host) ? host : '127.0.0.1';
}
...
interface SendOptions {
  timeoutMs: number;
  /** 테스트 주입용. 기본은 프로토콜에 맞는 node http(s) 의 request(lib/pinned-request.ts). */
  request?: PinnedRequestFn;
}

/** `/me` 응답 본문 상한 — 우리 api 라도 응답을 무한정 메모리에 담지 않는다. */
const IDENTITY_BODY_MAX_BYTES = 16 * 1024;
...
export async function postFeedbackToApi(
  ...
  { timeoutMs, request }: SendOptions,
): Promise<number> {
  // 연결은 확인한 IP 에 고정한다(리다이렉트도 따라가지 않는다 — 따라가면 서비스 키가 다른
  // 호스트로 다시 나간다. 3xx 는 상태 코드로 돌아와 호출부가 실패로 본다).
  // 응답 본문(`{ id, createdAt }`)은 화면에 쓰지 않으므로 읽지 않고 파기한다.
  const { status } = await requestPinned(target.endpoint, target.address, {
    method: 'POST',
    headers: {
...
export async function resolveUserId(
  ...
  try {
    // 저장 요청과 **같은 IP 에 고정**한다 — 쿠키가 나가는 요청이라 여기서 리바인딩이 열리면
    // 인증 쿠키가 내부 주소로 간다.
    const res = await requestPinned(target.identityEndpoint, target.address, {
      method: 'GET',
      headers: { cookie: forwarded, accept: 'application/json' },
      timeoutMs,
      readBody: true,
      maxBodyBytes: IDENTITY_BODY_MAX_BYTES,
      request,
    });
    // 200 이 아니면 "모른다" 다 — 401(미인증)·만료·오류·3xx 모두. 본문은 보지도 않는다.
    if (res.status !== 200) return null;
    const body = JSON.parse(res.body) as MeIdentity;
