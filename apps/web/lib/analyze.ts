...
export async function analyze(raw: unknown): Promise<AnalyzeResult> {
  const parsed = analysisInputSchema.parse(raw) // 입학연도·동의(sensitive===true) 검증
  ...
  const profile: AnalysisInput = { ...parsed, saengbu: maskedSaengbu }
  ...
  const priorProfile: AnalysisInput = { ...profile, saengbu: priorMaskedSaengbu }
