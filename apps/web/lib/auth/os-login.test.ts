...
import { osLoginHref, osSignupHref } from './os-login';
...
describe('osSignupHref (OS 가입 redirect + next 복귀 — login 과 패리티)', () => {
  it('NEXT_PUBLIC_OS_URL 설정 시 {OS}/signup?next=<returnUrl>', () => {
    vi.stubEnv('NEXT_PUBLIC_OS_URL', 'http://pullim.local:3001');
    expect(osSignupHref('http://os.pullim.local:3007/result/abc')).toBe(
      'http://pullim.local:3001/signup?next=http%3A%2F%2Fos.pullim.local%3A3007%2Fresult%2Fabc'
    );
  });

  it('trailing slash 있는 base도 정상(/signup 합성)', () => {
    vi.stubEnv('NEXT_PUBLIC_OS_URL', 'https://pullim.ai/');
    expect(osSignupHref('https://admissions.pullim.ai/submit')).toBe(
      'https://pullim.ai/signup?next=https%3A%2F%2Fadmissions.pullim.ai%2Fsubmit'
    );
  });

  it('NEXT_PUBLIC_OS_URL 미설정 → null(내부 /signup 폴백)', () => {
    vi.stubEnv('NEXT_PUBLIC_OS_URL', '');
    expect(osSignupHref('http://x/y')).toBeNull();
  });
