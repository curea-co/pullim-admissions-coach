 ...
 * ⚠️ 이름만 본다. 이름이 실제로 가리키는 주소는 resolvePublicAddress() 가 확인한다.
 */
...
/**
 * 이름이 **실제로 가리키는 주소**까지 확인하고, 연결에 쓸 주소 하나를 돌려준다.
 * 호스트명 문자열만 보면 `127.0.0.1.nip.io` 처럼 공인 도메인이 내부 주소로 해석되는 경우를
 * 그대로 통과시킨다 — 연결은 그 해석 결과로 이뤄지므로 https 조건만으로는 막히지 않는다.
 * 해석된 주소가 **하나라도** 내부면 거절하고, 해석 실패·빈 결과도 거절한다(모르면 보내지 않는다).
 *
 * 반환값을 그대로 연결 대상으로 써야 의미가 있다(lib/webhook-post.ts 의 IP 고정) — 확인만 하고
 * 이름으로 다시 연결하면 그 사이에 응답이 바뀌는 DNS 리바인딩을 막지 못한다.
 */
export async function resolvePublicAddress(
  ...
  if (/^[\d.]+$/.test(host) || host.includes(':')) return isInternalHost(host) ? null : host;
  try {
    const addresses = await lookup(host);
    if (addresses.length === 0) return null;
    if (addresses.some((address) => isInternalHost(address))) return null;
    return addresses[0];
  } catch {
    return null;
  }
